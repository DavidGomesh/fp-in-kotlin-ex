...
fun <A> Stream<A>.take(n: Int): Stream<A> =
    unfold(this) { s -> when(s) {
        is Empty -> None
        is Cons -> if (n == 0) None else Some(s.head() to s.tail().take(n - 1))
    } }

fun <A> Stream<A>.takeWhile(p: (A) -> Boolean): Stream<A> =
    unfold(this) { s -> when(s) {
        is Empty -> None
        is Cons -> if (p(s.head())) Some(s.head() to s.tail().takeWhile(p)) else None
    } }

fun <A, B, C> Stream<A>.zipWith(that: Stream<B>, f: (A, B) -> C): Stream<C> =
    unfold(this to that) { (xs1, xs2) -> when(xs1) {
        is Empty -> None
        is Cons -> when(xs2) {
            is Empty -> None
            is Cons -> Some(f(xs1.head(), xs2.head()) to (xs1.tail() to xs2.tail()))
        }
    } }

fun <A, B> Stream<A>.zipAll(that: Stream<B>): Stream<Pair<Option<A>, Option<B>>> =
    unfold(this to that) { (xs1, xs2) ->
        when(xs1) {
            is Empty -> when(xs2) {
                is Empty -> None
                is Cons -> Some((None to Some(xs2.head())) to (empty<A>() to xs2.tail()))
            }
            is Cons -> when(xs2) {
                is Empty -> Some((Some(xs1.head()) to None) to (xs1.tail() to empty()))
                is Cons -> Some((Some(xs1.head()) to Some(xs2.head())) to (xs1.tail() to xs2.tail()))
            }
        }
    }
//end::init[]
